(() => {
  ...
  const TOKEN_KEY = "light-tower-organizer-token";
  const EXPIRY_KEY = "light-tower-organizer-expiry";
  const logError = (message, error) => console.error(`[Guild Organizer Auth] ${message}`, error);
  ...
  const captureSessionFromHash = () => {
    try {
      ...
      logError("Could not capture organizer session.", error);
      return false;
    }
  };
  ...
  const getAccessToken = () => {
    try {
      ...
      logError("Could not read organizer session.", error);
      return null;
    }
  };
  ...
  const requestMagicLink = async (email) => {
    try {
      ...
      logError("Magic-link request failed.", error);
      throw error;
    }
  };
  ...
  const signOut = async () => {
    try {
      ...
      if (token) await fetch(`${PROJECT_URL}/auth/v1/logout`, {
        method: "POST",
  ...
  window.GuildOrganizerAuth = { captureSessionFromHash, getAccessToken, requestMagicLink, signOut };
})();