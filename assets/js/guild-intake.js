(() => {
  ...
  const PRODUCTION_HOST = "lighttowertabletopguild.netlify.app";
  const ENDPOINTS = {
  ...
  const previewBlocked = (event, errors) => {
    if (isProduction()) return false;
    event.preventDefault();
    showErrors(errors, ["Preview environment: submissions are disabled so test data cannot enter the production Guild intake."]);
    return true;
  };
  ...
  const submitWithState = async (button, errors, endpoint, payload) => {
  ...
  const setupAdultIntake = () => {
    ...
    form.addEventListener("submit", async (event) => {
      try {
        ...
        if (previewBlocked(event, errors)) return;
        event.preventDefault();
        await submitWithState(button, errors, ENDPOINTS.adult, payload);
  ...
  const setupYouthIntake = () => {
    ...
    form.addEventListener("submit", async (event) => {
      try {
        ...
        if (previewBlocked(event, errors)) return;
        event.preventDefault();
        await submitWithState(button, errors, ENDPOINTS.youth, payload);
  ...
  try { setupAdultIntake(); setupYouthIntake(); }
  catch (error) { logError("Guild intake could not be initialized.", error); }
})();
