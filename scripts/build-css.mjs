...
const bundles = {
  ...
  'assets/css/member-bundle.css': ['assets/css/base.css','assets/css/components.css','assets/css/responsive.css','assets/css/fantasy-theme.css','assets/css/guild-pages.css','assets/css/member.css'],
  'assets/css/core-bundle.css': ['assets/css/base.css','assets/css/components.css']
};

const pageBundles = {
  'index.html': 'assets/css/home-bundle.css','first-adventure.html': 'assets/css/adventure-bundle.css','character-sheet-guide.html': 'assets/css/character-sheet-bundle.css','one-shots.html': 'assets/css/one-shots-bundle.css','guild-hall.html': 'assets/css/guild-bundle.css','tools.html': 'assets/css/guild-bundle.css','join.html': 'assets/css/form-bundle.css','youth-groups.html': 'assets/css/form-bundle.css','organizer.html': 'assets/css/organizer-bundle.css','member.html': 'assets/css/member-bundle.css','404.html': 'assets/css/guild-bundle.css','thanks.html': 'assets/css/core-bundle.css'
};
...
for (const [output, inputs] of Object.entries(bundles)) {
  ...
  } catch (error) { console.error(`Failed to build ${output}:`, error); process.exitCode = 1; }
}

for (const [page, output] of Object.entries(pageBundles)) {
  try {
    let html = fs.readFileSync(page, 'utf8');
    for (const input of bundles[output]) { const tag = stylesheetTag(input); html = html.replace(`${tag}\n`, '').replace(tag, ''); }
    html = html.replace(`${stylesheetTag(output)}\n`, '').replace(stylesheetTag(output), '').replace(/\s*<style data-production-css>[\s\S]*?<\/style>/, '');
    const css = builtCss.get(output); const iconMarker = '<link rel="icon" href="assets/favicon.svg" type="image/svg+xml">';
  ...
  } catch (error) { console.error(`Failed to rewrite ${page}:`, error); process.exitCode = 1; }
}
